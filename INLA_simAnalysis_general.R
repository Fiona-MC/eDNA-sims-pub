...
# to run
# Rscript INLA_simAnalysis_general.R /home/fiona_callahan/simData/testing/run1/ /home/fiona_callahan/simData/testing/INLAres/
...
for (trial in 1:numTrials){
  ...
  for (response in names_species){ # NOTE -- this takes a few minutes
    ...
    reslist[[response]] <- RunInlaBin(response, sitetab, xytab, mesh_s, mesh_t, 1, 
                                namescov = c(), normcov = 0, project = proj)
  }
  ...
  for (response in names_species){ 
    ...
    # Run INLA 
    #response="Sp1"
    res_cov_only[[response]] <- RunInlaBin(response, sitetab, xytab, mesh_s, mesh_t, 1,
                                  namescov = names_cov, normcov = 0, project = proj)
  ...
  sim_lists <- list()
  sim_lists[["None"]] <- reslist
  #sim_lists[["None2"]] <- reslist1
  # ADD MODELS
  sim_lists[["AllCov"]] <- res_cov_only
  ...
  # I think because the no covariates model didnt work on all of the animals -- Not sure why though
  ...
  ggsave(best_WAIC, file = paste0(subdir, "bestWAIC_INLA-iid-", trial, ".png"))
  ...
  ggsave(cov_only, file = paste0(subdir, "covonly_INLA-iid-", trial, ".png"))
  ...
  ggsave(animal_cov, file = paste0(subdir, "animal_cov_INLA-iid-", trial, ".png"))
}
