...
for(trial in 1:6){
  subdir = paste0("./3sp_CovOnly_1b/trial", trial, "_orig/")
  dataDir = "./simData/3sp_CovOnly_1b/"
  dir.create(subdir)
  ...
  timePts<-seq(from=1, to=params$numGens, by = 100)
  ...
  for(t in timePts){
    for(s_index in 1:length(locList)){
      ...
      sitetab_data[i+4] <- sim_data_raw[[t]]$cov1[s_index]
      sitetab_data[i+5] <- sim_data_raw[[t]]$cov2[s_index]
      sitetab_data[i+6] <- sim_data_raw[[t]]$cov3[s_index]
  ...
  names(sim_sitetab)<-c("labID", "Age", "Lat", "Long", "Cov1", "Cov2","Cov3", "Sp1", "Sp2", "Sp3")# "Sp4", "Sp5")
  ...
  saveRDS(simLists, paste0(subdir, "simLists-iid",trial,".Rdata"))
  runData<-list()
  runData$params<-params
  runData$sim_data_raw<-sim_data_raw
  runData$sitetab<-sitetab
  saveRDS(runData,paste0(subdir,"rundata-iid",trial,".Rdata"))
  # CPO 
  ...
  # RESOLVED: THIS ISNT WORKING VVVVVVVVV --wasnt working because not all of the models had all of the animals in the list
  ...
  ggsave(model_plot, file=paste0(subdir,"model_comparisons-iid",trial,".png"))
  
  # Best model
  covarplots <- CreateCovarPlots(scoretab = waictab,names = goodanimalnames,modellist = simLists)
  bestWAIC<-grid.arrange(covarplots[[1]],covarplots[[2]],covarplots[[3]], vp=viewport(width=0.98, height=0.98))
  ggsave(bestWAIC, file=paste0(subdir,"bestWAIC_INLA-iid",trial,".png"))
  
  # Model without all cov NOT animal
  covarplots <- CreateCovarPlots(waictab,goodanimalnames,simLists,forcemodel="AllCov")
  covOnly<-grid.arrange(covarplots[[1]],covarplots[[2]], covarplots[[3]],vp=viewport(width=0.98, height=0.98))
  ggsave(covOnly, file=paste0(subdir,"covonly_INLA-iid",trial,".png"))
  
  # Model with cov and animal
  covarplots <- CreateCovarPlots(waictab,goodanimalnames,simLists,forcemodel="Cov+Animal")
  animalCov<-grid.arrange(covarplots[[1]],covarplots[[2]], covarplots[[3]],vp=viewport(width=0.98, height=0.98))
  ggsave(animalCov, file=paste0(subdir,"animalCov_INLA-iid",trial,".png"))
}
