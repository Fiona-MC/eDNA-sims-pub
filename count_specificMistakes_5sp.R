...
#names(alphaInferredDF) <- c("alpha11", "alpha21", "alpha31", "alpha41", "alpha51",
#                            "alpha12", "alpha22", "alpha32", "alpha32", "alpha32",
...
average_alphaInferred <- matrix(data = apply(X = alphaInferredDF[!is.na(alphaInferredDF$V1), ], MARGIN = 2, FUN = mean), 
                                nrow = nSpecies, ncol = nSpecies)

#average_betaInferred <- matrix(data = apply(X = betaInferredDF[!is.na(betaInferredDF$V1), ], MARGIN = 2, FUN = mean), 
 #                               nrow = nSpecies, ncol = nCov)

average_abs_alphaInferred <- matrix(data = apply(X = alphaInferredDF[!is.na(alphaInferredDF$V1), ], MARGIN = 2, 
                                FUN = function(x) {mean(abs(x))}), 
                                nrow = nSpecies, ncol = nSpecies)

#average_abs_betaInferred <- matrix(data = apply(X = betaInferredDF[!is.na(betaInferredDF$V1), ], MARGIN = 2, 
#                                FUN = function(x) {mean(abs(x))}), 
 #                               nrow = nSpecies, ncol = nCov)
